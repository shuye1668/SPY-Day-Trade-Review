...
COLORS_FILE=r"C:\TradeReview\colors.xlsx"
DIVIDENDS_FILE=r"C:\TradeReview\dividends.xlsx"
...
def _analyse_all_trades():
    ...
    if _any_changed or _rows_merged:
        try:
            for col in ("Action", "Status", "Pair_Date", "Pair_Time"):
...
function draw(){
  ...
  dayList.forEach((date,di)=>{
    ...
    const tradeBelongsOnPage=(t)=>{
      // Draw ALL trades on EVERY page so overlapping regions have identical content
      // (user cuts/pastes pages manually and needs trades visible on both sides of the cut)
      return true;
    };
    ...
    (day.trades||[]).forEach(t=>{
      ...
      let lx1=entryDi2>=0?xOfGi(gi(entryDi2,ei2)):M.l-20;
    ...
    // Sort: normal intraday first (by entryTime), then crossDay/isHold last
    ...
    tradesIndexed.forEach(({t,ti})=>{
      ...
      if(isHold){
        ...
        const entryDi=dayList.indexOf(t.entryDate);
        if(entryDi>=0){x1=xOfGi(gi(entryDi,ei));}
        else{x1=M.l-20;}
        x2=xOfGi(gi(di,xi));y2=yOf(t.exitPrice,pn,px);
      } else {
        x2=xOfGi(gi(di,xi));y2=yOf(t.exitPrice,pn,px);
      ...
      if(isHold){
        ...
        midX=(x1+x2)/2;
        baseAbove=Math.min(y1,y2)-lblH/2-6;
        baseBelow=Math.max(y1,y2)+lblH/2+6;
      }
      ...
      if(!chosen){
        const fx=(isHold||isCross)?Math.max(leftBound,midX-lblW/2):Math.max(leftBound,midX-lblW/2,prevLabelCenterX-lblW/2);
...
async function exportPNG(){
  ...
  exportMode=false;exportLightMode=false;exportDualAxis=false;exportYAxisLeft=false;exportYOverride=null;notesAnchorPrice=null;notesPageIdx=null;currentExportPageIdx=null;exportPageTops=null;exportPrevClose=null;
...
if __name__=="__main__":
    ...
    if os.path.exists(HISTORY_FILE):
        ...
        hist = _load_history_by_date()
        print(f"  ✓ {len(hist)} trading days ready")
