  ...
  dayList.forEach((date,di)=>{
    ...
    if(exportMode&&exportPageTops&&exportPageTops.length>1){
      if(exportLabelCache===null){
        ...
        const tIdx=(day.trades||[]).map((t,ti)=>({t,ti}));
        tIdx.sort((a,b)=>{
        ...
        tIdx.forEach(({t})=>{
          ...
          else{midX=(x1+x2)/2;baseAbove=Math.min(y1,y2)-lblH/2-6;baseBelow=Math.max(y1,y2)+lblH/2+6;}
          const candidates=[];
          ...
          if(!chosen){
            const fx=(isHold||isCross)?Math.max(leftBound,midX-lblW/2):Math.max(leftBound,midX-lblW/2,prevCX-lblW/2);
