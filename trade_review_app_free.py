"""
Trade Review — Web App (FREE / No-Bloomberg version)
=====================================================
Reads SPY 1-minute candles from local JSON cache (populated by daily_fetch.py).
...
Folder layout:
  C:\\TradeReview\\
    ...
    candles\\
      SPY_2026-04-08.json     ← per-day OHLC cache (auto-fetched by daily_fetch.py)
...
# In-memory cache for parsed candles (to avoid re-reading JSON every request)
_cache={}

def fetch_intraday(ticker,date_str):
    """Read 1-minute candles for a given date from local JSON cache.
    Returns a list of {t, o, h, l, c, v} dicts, or [] if no data."""
    k=f"{ticker}|{date_str}"
    if k in _cache:return _cache[k]
    path=os.path.join(CANDLES_FOLDER,f"{ticker}_{date_str}.json")
    ...
    try:
        ...
        print(f"[fetch_intraday] failed to read {path}: {e}")
        return []
...
def read_and_pair_for_date(date_str):
    """Filter trades_all.xlsx by Date column and pair entries with exits.
    Returns a list of trade dicts (same shape as Bloomberg version)."""
    df=load_trades_df()
    if df is None or len(df)==0:return []
    if "Date" not in df.columns:
        print("[read_and_pair_for_date] trades_all.xlsx is missing Date column")
        return []
    # Normalize Date column to YYYY-MM-DD strings for comparison
    def norm_date(v):
        ...
        # Already datetime-ish from Excel?
        try:return pd.to_datetime(s).strftime("%Y-%m-%d")
        except:return s
    sub=df.copy()
    sub["_d"]=sub["Date"].apply(norm_date)
    sub=sub[sub["_d"]==date_str]
    if len(sub)==0:return []
    # Same pairing logic as the Bloomberg version
...
@app.route("/api/dates")
def api_dates():
    """Return all dates that have either candle data OR trade entries."""
    dates=set()
    # From candle JSON files
    if os.path.isdir(CANDLES_FOLDER):
        for f in glob.glob(os.path.join(CANDLES_FOLDER,f"{TICKER}_*.json")):
    ...
    # From trades_all.xlsx Date column
    df=load_trades_df()
    if df is not None and "Date" in df.columns:
        for v in df["Date"].dropna().unique():
            ...
            for fmt in ("%Y-%m-%d","%Y/%m/%d","%m/%d/%Y","%Y%m%d"):
                try:
                    nd=dt.datetime.strptime(s,fmt).strftime("%Y-%m-%d")
                    dates.add(nd);break
                except:pass
            else:
                try:dates.add(pd.to_datetime(s).strftime("%Y-%m-%d"))
                except:pass
...
if __name__=="__main__":
    ...
    if not os.path.exists(CANDLES_FOLDER):os.makedirs(CANDLES_FOLDER,exist_ok=True)
    if not os.path.exists(TRADES_FILE):
