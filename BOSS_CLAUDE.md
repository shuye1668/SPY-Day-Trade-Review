...
## ⚠️ ExecutionPolicy：這台會擋 .ps1

這台的 ExecutionPolicy 會擋住 `.ps1`，**兩種寫法都會失敗**：

```powershell
. C:\TradeReview_Boss\tos_automation.ps1     # ✗ 因為這個系統上已停用指令碼執行
.\sync_push.ps1 -Owner boss                  # ✗ 同上
```

**解法：不改系統設定，每次呼叫時繞過即可。**

- 需要在**目前這個 session 內**保留函式（例如 `tos_automation.ps1` 的
  `Activate-TOSWindow`／`Click-At`）→ 每個 PowerShell tool call 開頭先加：
  ...
  `-Scope Process` 只影響這個 process，不動系統、不需提權、視窗關掉就沒了。
  每次 tool call 都是新 process，所以**每個 call 都要重下這兩行**。

- 只是要**執行一支獨立腳本** → 用 `-File` 形式：

  ```powershell
  powershell -ExecutionPolicy Bypass -File C:\TradeReview\sync_push.ps1 -Owner boss
  ```

本檔以下所有 `.\xxx.ps1` 的寫法，都請照上面兩種形式之一改寫。
...
- 不可用 `git add -A` —— 一律用 `powershell -ExecutionPolicy Bypass -File C:\TradeReview\sync_push.ps1 -Owner boss`
- 不可為了讓流程跑完而修改任何數字
